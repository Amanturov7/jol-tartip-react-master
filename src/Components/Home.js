...
const Home = () => {
  ...
  const fetchRecentApplications = async () => {
      try {
          const response = await Axios.get('https://jortartip.onrender.com/rest/applications/latest');
  ...
  const fetchRecentReviews = async () => {
    try {
        const response = await Axios.get('https://jortartip.onrender.com/rest/reviews/latest');
  ...
  const renderApplications = () => {
      return recentApplications.map((application) => (
        <Link to={`/applications/${application.id}`} key={application.id} className="application-box">
          <img src={`https://jortartip.onrender.com/rest/attachments/download/applications/${application.id}`} alt={`Application ${application.id}`} />
