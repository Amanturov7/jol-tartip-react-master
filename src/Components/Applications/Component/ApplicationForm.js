...
const ApplicationForm = ({ onCancel }) => {
  ...
  useEffect(() => {
    ...
    const fetchRegions = async () => {
      try {
        const response = await Axios.get('https://jortartip.onrender.com/rest/common-reference/by-type/001');
  ...
  useEffect(() => {
    const fetchDistrictsByRegionId = async () => {
      try {
        const response = await Axios.get(`https://jortartip.onrender.com/rest/common-reference/parent/${regionId}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const applicationResponse = await Axios.post(
        'https://jortartip.onrender.com/rest/applications/create',
        newApplication,
        {
          headers: {
